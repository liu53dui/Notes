...
* 第十六题
  ...
  var deleteDuplicates = function(head) {
    let before = head;
    while(before) {
        let back = before.next;
        while (back&& before.val == back.val){
            back = back.next
        }
        before.next = back
        before = back
...
* 第十七题
  ...
  #解题思路：因为是两个有序的数组，而且nums1的长度是填充了的，虽然他的初始长度为m但是后面补充了n个0，所以只需要从后往前判断把最大的值往后放即可。
  class Solution:
    def merge(self, nums1: List[int], m: int, nums2: List[int], n: int) -> None:
        while m > 0 and n >0:
            if nums1[m-1] < nums2[n-1]:
                ...
                m = m-1
        if m == 0 and n > 0:
            nums1[:n] = nums2[:n] #把nums2的前n个数字放到nums1的前n个里       
  ```
  ```javascript
